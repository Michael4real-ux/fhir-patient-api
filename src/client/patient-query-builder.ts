...
import {
  ...
  RegisterResource,
} from '../extensibility/resource-factory';
...
@RegisterResource<Patient, PatientSearchParams>({
  ...
  validateSearchParams: (params: PatientSearchParams) => {
    ...
    if (
      params.gender &&
      !['male', 'female', 'other', 'unknown'].includes(params.gender)
    ) {
...
export class PatientQueryBuilder extends FactoryResourceQueryBuilder<
  Patient,
  PatientSearchParams
> {
   ...
   */
  clone(): this {
    const cloned = new PatientQueryBuilder(
      this.baseUrl,
