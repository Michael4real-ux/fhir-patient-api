...
import { FHIRNetworkError, FHIRServerError, CircuitBreakerError } from '../errors';
...
describe('CircuitBreaker', () => {
  ...
  describe('error percentage threshold', () => {
    it('should open circuit based on error percentage', async () => {
      ...
      // Execute requests in mixed order to reach volume threshold
      // 7 failures, 3 successes (70% error rate)
      const requests = [
        ...Array(7).fill('fail'),
        ...Array(3).fill('success')
      ];
      
      // Shuffle to avoid opening circuit early
      ...
      for (const request of requests) {
        ...
        // Check if circuit opened early due to error percentage
